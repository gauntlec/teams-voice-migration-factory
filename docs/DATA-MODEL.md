...
| `tenant_discovery_config` | one row: `filter_users` boolean (default true — a run stores only `User` accounts licensed for Teams; set false per customer to store every enabled user), updated_by/at |
