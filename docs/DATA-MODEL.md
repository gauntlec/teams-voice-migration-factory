...
| `tenant_objects` | **current snapshot**, one row per object: `object_type` (`TENANT_OBJECT_TYPES` — includes `teams_device`: phones / rooms / panels / SIP from Graph `/teamwork/devices`), `object_key` (unique per type), display_name, `data` jsonb (GIN), generated `search` tsvector (GIN), first/last_seen_run_id, discovered_at, `content_changed_at` (when `data` last actually changed), `removed_at` (tombstone when a later successful run no longer returns it) |
...
| Table | Notes |
|-------|-------|
| `connections` | id, started_by, method (`device_code`), status (`pending`/`active`/`expired`/`closed`), user_code, verification_uri, tenant_domain, upn, scopes, started_at, expires_at, closed_at, **`graph_status`** (`none`/`pending`/`active`/`failed`) + `graph_user_code`/`graph_verification_uri`/`graph_upn`/`graph_expires_at` (the optional 2nd Microsoft Graph sign-in for the Teams device inventory, migration 0014) — **no tokens** |
